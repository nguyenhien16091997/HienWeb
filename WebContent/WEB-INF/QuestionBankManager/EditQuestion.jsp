...
<body>
				      	...
				      	<li class="dropdown">
					        ...
					        <ul class="dropdown-menu">
					        	<li><a href="loginController"><img src="Image/home-icon.png" style="width: 20px; height: 20px">Trang chủ</a><hr width="100%" style="margin: 0"/></li>					          	
					          	<li><a href=""><img src="logoutController" style="width: 20px; height: 20px;">Đăng xuất</a></li>
					        </ul>					        
				     	</li>				     
	...
	<div class="container" id="BlockBody" style="overflow: auto; height: 600px">	
	    ...
	    <div class="col-sm-10" style="height: 500px">    	 
	    	<p style="text-align: center;"><strong><span class="glyphicon glyphicon-pencil" style="margin-right: 5px"></span>CHỈNH SỬA CÂU HỎI</strong></p>
	 		<hr width="90%" align="left" />	
	 		<form class="form" action="updateController" method="post">
	 			<div class="row">
	 				<div class="col-sm-6"> 				
					      	<input name="questionID" value="${param.questionID }" style="display: none" />				      
					      ...
					      <div class="col-sm-7">
					            <select class="form-control" id="subjectChose" style="margin-bottom: 20px" name="subjectID">
					            	<option value="${mapQuestion.get(questionID).getSubjectID() }"> ${mapSubject.get(mapQuestion.get(questionID).getSubjectID()).getSubjectName() } </option>
					      ...
					      <div class="col-sm-7">
					            <select class="form-control" id="themeChose" style="margin-bottom: 20px" name="themeID">							   
							      	<option value="valueOld">${mapQuestion.get(questionID).getThemeID() }</option>		        
							    </select>
					      </div>
					      ...
					      <div class="col-sm-7">					      
					            <select class="form-control" id="Level" style="margin-bottom: 20px" name="levelID">
					            	<option value="${mapQuestion.get(questionID).getLevelID() }">${mapLevel.get(mapQuestion.get(questionID).getLevelID()).getLevelName() }</option>
					...
					<div class="col-sm-6" >
						<label class="control-label col-sm-3" for="IDQuestion" style="margin-top: 5px" >Nội dung:</label>
					      <div class="col-sm-7">
					            <textarea class="form-control" id="IDQuestion" style="margin-bottom: 20px" required="required" name="content">${mapQuestion.get(questionID).getContent() }</textarea>						       
					      </div>
					       <div class="col-sm-2">
					       </div>
					      
						      <label class="control-label col-sm-12" for="A" style="margin-top: 5px">Câu trả lời:</label>
						      
						    <c:forEach var="answerFollowQuestionID" items="${requestScope.listAnswerFollowQuestionID }">

						    <div id="divH${answerFollowQuestionID.getAnswerID() }">						    
						    	<button onclick="DL(${answerFollowQuestionID.getAnswerID() })" style="margin-top: 5px" class="btn btn-default btn-sm control-label col-sm-3 ">xóa câu trả lời</button>
							    <div class="col-sm-2"><input class="form-control answerClass" id="A1" style="margin-bottom: 20px" placeholder="vd: A..." name="answerName${answerFollowQuestionID.getAnswerID() }" value="${answerFollowQuestionID.getAnswerName() }" required="required">
							    </div>
							    <div class="col-sm-5"><input class="form-control" id="B1" style="margin-bottom: 20px" placeholder="Nội dung câu trả lời..." name="answerContent${answerFollowQuestionID.getAnswerID() }" value="${answerFollowQuestionID.getAnswerContent() }" required="required">
							    </div>
							    <div class="col-sm-2">
							    </div>
							</div>	
					       ...
					       <div class="col-sm-3">
					         ...
					         <div class="col-sm-7">
					         	<input id="addAnswer" type="button" class="btn btn-default btn-sm" value=" Thêm câu trả lời " style="font-size: 15px ;color: #090808; background-color: #D2C092; margin-bottom: 20px" required="required" />
					     ...
					     <div class="col-sm-2">
					      ...
					      <div class="col-sm-3">
					            <input class="form-control isCorrectClass" onblur="check()" id="A" style="margin-bottom: 20px" placeholder="vd: A..." name="isCorrect" value="${answerNameCorrect }" required="required" >	
					      </div>
					      <div class="col-sm-7">
					       </div> 					      
					</div> 				
	 			...
	 			<div class="form-group">				    
					...
					<div class=" col-sm-3">
					     <button id="save" type="submit" class="btn btn-default" style="color: #F9D1AE; background-color: #161514" name="command" value="updateQuestion">Lưu thay đổi</button>
	...
	var access= function(){
		...
		if($("#subjectChose").val() == ${sub.getSubjectID()})
			{
			if(${sub.getSubjectID()== mapQuestion.get(questionID).getSubjectID()}){
				
				$("#themeChose").html(function(){
					var rs="";
					rs+="<option value='${mapQuestion.get(questionID).getThemeID()}'>${mapTheme.get(mapQuestion.get(questionID).getThemeID()).getThemeName()}</option>"
 ...
 /* =============== alert Annount ============  */
	if(${not empty param.statusUpdate}){
		if(${param.statusUpdate==true}){
			...
			window.location.href = 'updateController?command=updateQuestion&questionID=${param.questionID}'; 
		}
		else{
			alert("sorry !!! không sửa được rồi ");
			window.location.href = 'updateController?command=updateQuestion&questionID=${param.questionID}'; 
		}
	}
