	 				...
	 				<div class="input ">	 	
	 					<c:forEach var="answer" items="${answerDAO.getListAnswerFollowQuestionID(testQuestion.getQuestionID()) }">
	 						<c:if test="${answer.getIsCorrect()==true }">
	 							<input  type="radio" disabled="disabled" checked="checked"/> ${answer.getAnswerContent() }<br>	
	 						</c:if>
	 						<c:if test="${answer.getIsCorrect()==false }">
	 							<input  type="radio" disabled="disabled" /> ${answer.getAnswerContent() }<br>
	 						</c:if>
...
if(${not empty statusDetail}){
	if(${statusDetail=="false"}){
		alert("sorry :) Đề thi chưa có câu hỏi!!!");
		window.location.href = 'examContent'; 
	}
